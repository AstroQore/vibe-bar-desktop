...
1. `QuotaEngine::refresh` fetches every provider with a live adapter
   (Codex, Claude, Alibaba, Z.ai, MiniMax, Kilo, OpenRouter, and Warp today),
   in sequence, each with a 30-second ceiling.
...
Desktop fetches eight providers and reads twenty-five. A number it fetched and
