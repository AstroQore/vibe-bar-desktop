...
1. `QuotaEngine::refresh` fetches every provider with a live adapter
   (Codex, Claude, Z.ai, MiniMax, OpenRouter, and Warp today), in sequence,
   each with a 30-second ceiling.
...
Desktop fetches six providers and reads twenty-five. A number it fetched and
